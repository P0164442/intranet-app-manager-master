...
public class UserController {
    ...
    private BaseResponse login(HttpServletRequest request, String username, String password) {
        ...
        try {
            ...
            return ResponseUtil.fail(USER_INVALID_ACCOUNT, "ユーザアカウントまたはパスワードが不正です。");
        } catch (LockedAccountException lae) {
            return ResponseUtil.fail(USER_INVALID_ACCOUNT, "ユーザアカウントがロックされています。利用できません。");

        } catch (AuthenticationException ae) {
            return ResponseUtil.fail(USER_INVALID_ACCOUNT, "認証に失敗しました");
        }
